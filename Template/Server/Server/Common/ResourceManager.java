...
public class ResourceManager extends LockManager implements IResourceManager
{
    ...
    public  boolean commit(int xid) throws RemoteException, InvalidTransactionException
    {
        ...
        synchronized(local) {
            ...
            else
            {
                ...
                try {
                    storeTime( (System.nanoTime() - startTimes.get(xid)) / 1e6 );
                }
                catch (IOException e) {
    ...
    {
        startTimes.put(xid, System.nanoTime());
        boolean result = reserveItem_CustomerRM(xid, customerID, Flight.getKey(flightNum), String.valueOf(flightNum), price);
        try {
            storeTime( (System.nanoTime() - startTimes.get(xid)) / 1e6 );
        }
        catch (IOException e) {
    ...
    {
        startTimes.put(xid, System.nanoTime());
        boolean result = reserveItem_CustomerRM(xid, customerID, Car.getKey(location), location, price);
        try {
            storeTime( (System.nanoTime() - startTimes.get(xid)) / 1e6 );
        }
        catch (IOException e) {
    ...
    {
        startTimes.put(xid, System.nanoTime());
        boolean result = reserveItem_CustomerRM(xid, customerID, Room.getKey(location), location, price);
        try {
            storeTime( (System.nanoTime() - startTimes.get(xid)) / 1e6 );
        }
        catch (IOException e) {
